<?php
// ENSURE THIS SCRIPT IS ACCESSIBLE BY TRACK [API] USERS VIA FETCH / AJAX
...
function get_reverse_dns($ip) {
    ...
    if (function_exists('apcu_fetch') && $cached = apcu_fetch($cache_key)) return $cached;
    $hostname = gethostbyaddr($ip) ?: "Lookup failed";
    if (function_exists('apcu_store')) apcu_store($cache_key, $hostname, 3600);
    return $hostname;
}
...
function sanitize_input($value, $type = 'string') {
    ...
    switch ($type) {
        case 'int': return filter_var($value, FILTER_VALIDATE_INT) ?: 0;
        case 'float': return filter_var($value, FILTER_VALIDATE_FLOAT) ?: 0.0;
        case 'bool': return filter_var($value, FILTER_VALIDATE_BOOLEAN);
        default: return htmlspecialchars(strip_tags($value), ENT_QUOTES, 'UTF-8');
    }
}

// Extract and sanitize tracking data
$tracking_data = [
    'user_id' => USER_ID,
    ...
    'cookies_enabled' => sanitize_input($input['cookiesEnabled'] ?? 'Unknown', 'bool'),
    'cpu_cores' => sanitize_input($input['cpuCores'] ?? 0, 'int'),
...
function check_ip_reputation($ip) {
    if (!filter_var($ip, FILTER_VALIDATE_IP)) {
        return ['is_vpn'=>0,'is_tor'=>0,'is_proxy'=>0,'ASN'=>'Invalid','ISP'=>'Invalid','country'=>'Invalid','latitude'=>0.0,'longitude'=>0.0];
    }
    ...
    $default = ['is_vpn'=>0,'is_tor'=>0,'is_proxy'=>0,'ASN'=>'Unknown','ISP'=>'Unknown','country'=>'Unknown','latitude'=>0.0,'longitude'=>0.0];

    if ($http_code !== 200 || !$response) return $default;
    ...
    if (function_exists('apcu_store')) apcu_store($cache_key, $result, 3600);
...
try {
    ...
    $stmt = $conn->prepare("
        INSERT INTO logs 
        (user_id, website_id, ip, real_ip, reverse_dns, webrtc_ip, dns_leak_ip, user_agent,
         screen_resolution, language, timezone, cookies_enabled, cpu_cores, ram, gpu, battery,
         referrer, plugins, digital_dna, is_vpn, is_tor, is_proxy, ASN, ISP, country, latitude, longitude, timestamp)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
    ");
    ...
    $stmt->bind_param(
        ...
        $ip_reputation['ASN'],
        $ip_reputation['ISP'],
        $ip_reputation['country'],
        $ip_reputation['latitude'],
        $ip_reputation['longitude']
    );
